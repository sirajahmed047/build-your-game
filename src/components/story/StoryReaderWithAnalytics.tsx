...
import { useAnalytics } from '@/lib/hooks/useAnalyticsTracking'
import type { Choice, GameState } from '@/types/story'

interface StoryReaderWithAnalyticsProps {
  storyText: string
  choices: Choice[]
  gameState?: GameState
  genre: string
...
export function StoryReaderWithAnalytics({
  storyText,
  choices,
  gameState,
  genre,
  ...
  onToggleStats,
  showStats = false
}: StoryReaderWithAnalyticsProps) {
  const [expandedChoice, setExpandedChoice] = useState<string | null>(null)
  const [hasTrackedStepView, setHasTrackedStepView] = useState(false)
  ...
  // Track step view when component mounts or step changes
  useEffect(() => {
    if (!hasTrackedStepView) {
      track('story_step_viewed', {
        genre,
      ...
      setHasTrackedStepView(true)
    }
  }, [currentStep, genre, totalSteps, storyRunId, choices.length, hasTrackedStepView, track])

  // Reset tracking flag when step changes
  useEffect(() => {
    setHasTrackedStepView(false)
  }, [currentStep])

  const handleChoiceSelect = (choice: Choice) => {
    // Track the choice selection
    trackChoiceMade(
      choiceSlug || `step_${currentStep}_choice`,
      choice.id,
      currentStep,
      genre
    )

    // Track trait impact if present
    if (choice.traits_impact && Object.keys(choice.traits_impact).length > 0) {
      track('choice_with_traits_selected', {
        genre,
        stepNumber: currentStep,
        choiceSlug: choiceSlug || `step_${currentStep}_choice`,
        selectedOption: choice.id,
        traitsImpacted: Object.keys(choice.traits_impact),
        traitsImpactCount: Object.keys(choice.traits_impact).length
      })
    }
  ...
  const handleToggleStats = () => {
    if (onToggleStats) {
      // Track stats viewing
      if (!showStats && choiceSlug) {
        trackChoiceStatisticsViewed(choiceSlug, genre)
      }
      
      track('choice_statistics_toggled', {
        genre,
        stepNumber: currentStep,
        choiceSlug,
  ...
  const handleTraitExpansion = (choiceId: string, isExpanding: boolean) => {
    setExpandedChoice(isExpanding ? choiceId : null)
    
    // Track trait impact viewing
    track('trait_impact_viewed', {
      genre,
      stepNumber: currentStep,
      choiceId,
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              {hasTraitImpact && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    handleTraitExpansion(choice.id, !isExpanded)
                                  }}
